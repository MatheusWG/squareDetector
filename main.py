...
cv2.createTrackbar("Threshold1", "Parameters", 200, 1000, empty)
...
while True:
    ...
    lines = cv2.HoughLinesP(image=imgCanny, rho=3, theta=np.pi / 200, threshold=1, lines=np.array([]), minLineLength=40, maxLineGap=10)
    if lines is None:
        break
    a, b, c = lines.shape
    for i in range(a):
